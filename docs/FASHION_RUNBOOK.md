...
| | Full run | Rebuild |
|---|---|---|
| Command | `python -m backend.fashion.batch` | `python scripts/rebuild_catalog.py` |
...
PYTHONPATH=src python -m backend.fashion.batch \
  ...
  --locale      en-US \
  --decisions   shared/decisions/products_extended.jsonl   # optional
```
...
Details in [Conflict Resolution](FASHION_CONFLICT_RESOLUTION.md).
...
| | Attributes |
|---|---|
| all products | `primary_color`, `pattern`, `composition`, `care` |
...
[Reviewed Decisions](FASHION_DECISIONS.md).
