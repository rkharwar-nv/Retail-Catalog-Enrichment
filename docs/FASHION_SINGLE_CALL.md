...
`backend.fashion_enrich` enriches one fashion product with **one** vision-language
model call, against any OpenAI-compatible endpoint.

It is a self-contained alternative to the batch pipeline in
`backend.fashion`, not a replacement for it. Both are in the tree because they
answer different questions.

## Which one to use

| | `backend.fashion` (batch) | `backend.fashion_enrich` (single call) |
|---|---|---|
...
[Conflict Resolution](FASHION_CONFLICT_RESOLUTION.md).

## Usage

```python
from backend.fashion_enrich import enrich_product
